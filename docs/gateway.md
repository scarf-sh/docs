...
For lower-level package types like File Packages and Event Collection Packages, Scarf will look for system-recognized variables can be populated by using specific variables names by default, or you can configure them directly by setting a [custom variable override](https://app.scarf.sh/organizations/default/filters).
